...
[<Sealed>]
type Single<'R, 'A, 'B>(a: _2<'R, 'A, 'B>) =
  inherit TFingerTree<'R, 'A, 'B>()
  member val A = a

[<Sealed>]
type Deep<'R, 'A, 'B, 'C, 'D>(prefix: Digit<'R, 'A, 'B>, middle: TFingerTree<'R, 'B, 'C>, suffix: Digit<'R, 'C, 'D>) =
...
module TFingerTree =
  ...
  let single<'R, 'A, 'B>(a: _2<'R, 'A, 'B>) = Single<'R, 'A, 'B>(a)  :> TFingerTree<_, _, _>

  let deep<'R, 'A, 'B, 'C, 'D> (prefix: Digit<'R, 'A, 'B>) (middle: TFingerTree<'R, 'B, 'C>) (suffix: Digit<'R, 'C, 'D>) =
    Deep(prefix, middle, suffix) :> TFingerTree<_, _, _>
